...
- **`packages/env/`** (`@gainster/env`) — Centralized environment variable loading and validation. Reads root `.env` file, validates via zod schema, and exports a typed `Env` object. Used by scripts and smoke tests; library packages accept explicit config instead of reading env directly.
...
All env vars live in the root `.env` file. Use `loadEnv()` from `@gainster/env` to load and validate them — returns a typed `Env` object. Never commit `.env` files.
